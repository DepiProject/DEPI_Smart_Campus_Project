    ...
    async loadExams() {
        ...
        try {
            ...
            if (response.success && response.data) {
                ...
                if (exams.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted">No exams found</td></tr>';
                    return;
                }

                tbody.innerHTML = exams.map(exam => {
                    const examId = exam.examId || exam.id;
                    const examTitle = exam.examTitle || exam.title || 'Untitled Exam';
                    const courseName = exam.courseName || exam.course?.name || 'N/A';
                    const instructorName = exam.instructorName || exam.instructor?.name || 'N/A';
                    const duration = exam.durationMinutes || exam.duration || '-';
                    const totalMarks = exam.totalMarks || exam.marks || '-';
                    ...
                    return `
                        <tr>
                            ...
                            <td>${duration}</td>
                            <td>${totalMarks}</td>
                            <td><span class="badge bg-info">${questionsCount}</span></td>
                            <td>
                                <button class="btn btn-sm btn-info me-1" title="View Details" onclick="adminDashboard.viewExam('${examId}')">
                                    <i class="bi bi-eye"></i>
                                </button>
                                <button class="btn btn-sm btn-danger" title="Delete" onclick="adminDashboard.deleteExam('${examId}')">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </td>
                        </tr>
                ...
                tbody.innerHTML = '<tr><td colspan="7" class="text-center text-danger">Failed to load exams</td></tr>';
            }
        } catch (error) {
            console.error('Error loading exams:', error);
            tbody.innerHTML = '<tr><td colspan="7" class="text-center text-danger">Error loading exams</td></tr>';
        }
    }
